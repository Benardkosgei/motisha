...
export function CourseForm({ initialData = {}, onSuccess, mode }: CourseFormProps) {
  ...
  useEffect(() => {
    fetchAcademicTerms()
      ...
      .catch(() => {/* non-fatal — fall back to free-text */});
  }, []);
  ...
  function validate() {
    ...
    if (!modules || isNaN(modulesNum) || modulesNum <= 0) {
      e.modules = 'Number of modules must be greater than 0';
    }
    setErrors(e);
  ...
  async function doSubmit(action: 'draft' | 'publish') {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        publish_at:
          action === 'publish' ? now : publishAt ? fromEATInputValue(publishAt) : null,
  ...
  function handleSubmit(action: 'draft' | 'publish') {
    ...
    if (
      action === 'draft' &&
      publishAt &&
      new Date(fromEATInputValue(publishAt)) < new Date()
    ) {
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      {/* Description */}
      <div style={fieldStyle}>
        <label htmlFor="course-description" style={labelStyle}>
          Description
        </label>
        <textarea
      ...
      {/* Modules */}
      <div style={fieldStyle}>
        <label htmlFor="course-modules" style={labelStyle}>
          <Layers size={13} /> Number of Modules <span style={{ color: C.danger }}>*</span>
        </label>
        <input
          id="course-modules"
          type="number"
          ...
          style={{
            ...inputStyle,
            width: 160,
            borderColor: errors.modules ? C.danger : 'rgba(14,165,233,0.25)',
          }}
          disabled={submitting}
          ...
          {EMOJI_OPTIONS.map((emoji) => (
            <button
              ...
              style={{
                ...
                height: 38,
                borderRadius: 8,
                border: `2px solid ${icon === emoji ? C.teal : 'rgba(14,165,233,0.2)'}`,
                background: icon === emoji ? `${C.teal}20` : C.navyLight,
                fontSize: '1.2rem',
                cursor: 'pointer',
                display: 'flex',
                alignItems: 'center',
      ...
      <div style={fieldStyle}>
        ...
        {terms.length > 0 ? (
              ...
              <select
                ...
                onChange={e => {
                  ...
                  if (t) {
                    ...
                    setWeek(''); // reset week when term changes
                  }
                }}
                disabled={submitting}
          ...
          /* Fallback: no terms configured yet — free-text input */
          <div>
            <input
              id="course-week"
              type="text"
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              ...
              <h3
                id="past-date-title"
                style={{ color: C.mustard, margin: 0, fontSize: '1rem', fontWeight: 700 }}
              >
            ...
            >
              The publish date ({publishAt ? formatEAT(fromEATInputValue(publishAt)) : ''}) is in
              the past. Content will be published on the scheduler&apos;s next run. Continue?
            </p>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                onClick={() => {
                  setShowPastDateWarning(false);
                  if (pendingAction) doSubmit(pendingAction);
                }}
                style={{
                  padding: '9px 20px',
                  borderRadius: 8,
