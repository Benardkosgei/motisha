...
function DeleteDialog({ speech, onConfirm, onCancel, deleting }: {
  speech: Speech; onConfirm: () => void; onCancel: () => void; deleting: boolean;
}) {
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="del-title"
      ...
      <div style={{ background: C.navyMid, border: `1px solid ${C.danger}40`, borderRadius: 14, padding: 28, maxWidth: 420, width: '90%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          ...
          <h3 id="del-title" style={{ color: C.danger, margin: 0, fontSize: '1rem', fontWeight: 700 }}>Delete Speech</h3>
        </div>
        <p style={{ color: C.offWhite, fontSize: '0.85rem', lineHeight: 1.6, marginBottom: 20 }}>
          Delete <strong style={{ color: C.white }}>"{speech.title}"</strong>? This cannot be undone.
        </p>
...
export default function SpeechesPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<Speech | null>(null);
  ...
  const fetchSpeeches = useCallback(async () => {
    ...
    try {
      // Fetch all records — TanStack Table handles client-side pagination/search
      const res = await fetch('/api/admin/speeches?page=1&pageSize=1000');
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/admin/speeches/${deleteTarget.id}`, { method: 'DELETE' });
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.error || 'Failed'); }
      setSpeeches(prev => prev.filter(s => s.id !== deleteTarget.id));
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <RowActionsMenu
            viewHref={`/admin/speeches/${row.original.id}/edit`}
            editHref={`/admin/speeches/${row.original.id}/edit`}
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '1.4rem', fontWeight: 800, color: C.white, fontFamily: "'DM Sans',sans-serif" }}>🎤 Speeches</h1>
          <p style={{ margin: '4px 0 0', color: C.gray, fontSize: '0.82rem' }}>
            {loading ? 'Loading…' : `${speeches.length} speech${speeches.length !== 1 ? 'es' : ''} total`}
          </p>
        </div>
      ...
      {error && (
        <div role="alert" style={{ padding: '10px 16px', borderRadius: 8, background: `${C.danger}18`, border: `1px solid ${C.danger}40`, color: C.danger, fontSize: '0.85rem', marginBottom: 16 }}>
          {error}
          <button type="button" onClick={fetchSpeeches} style={{ marginLeft: 12, background: 'none', border: 'none', color: C.teal, cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600, textDecoration: 'underline', padding: 0 }}>Retry</button>
        </div>
      )}
      ...
      <AdminDataTable
        ...
        emptyMessage={<>No speeches yet. <Link href="/admin/speeches/new" style={{ color: C.teal, textDecoration: 'none', fontWeight: 600 }}>Create the first one →</Link></>}
      />

      {deleteTarget && (
        <DeleteDialog speech={deleteTarget} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} deleting={deleting} />
      )}
    </div>
  );
}
