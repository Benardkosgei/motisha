...
export async function PATCH(
  ...
  try {
    ...
    if (error) {
      console.error('[admin/speeches/[id]] PATCH error:', error.message || error, 'Code:', error.code);
      if (error.code === 'PGRST116') {
        return NextResponse.json({ error: 'Speech not found' }, { status: 404 });
      }
    ...
    console.error('[admin/speeches/[id]] PATCH error details:', error instanceof Error ? error.message : String(error));
    return NextResponse.json(
